...
export function formatQuantity(quintals) {
  ...
  return `${formatNumber(quintals, Number.isInteger(Number(quintals)) ? 0 : 2)} quintal`
}
...
export function relativeDay(value) {
  ...
  if (target === toISODate(today)) return 'Today'
  if (target === toISODate(tomorrow)) return 'Tomorrow'
