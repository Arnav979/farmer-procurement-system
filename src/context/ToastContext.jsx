...
export function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={value}>
      {children}
      <div
        className="print-hide pointer-events-none fixed inset-x-0 top-2 z-50 flex flex-col items-center gap-2 px-3"
        role="status"
        aria-live="polite"
      >
        {toasts.map((toast) => (
          <div
            key={toast.id}
            className={`pointer-events-auto flex w-full max-w-md items-start gap-3 border-l-4 bg-white px-4 py-3 shadow-md ${TONE_STYLES[toast.tone] || TONE_STYLES.info}`}
          >
